...
def Candy_bot (total_candy:int,max_candy_round:int): #"Интеллект" бота заключается в том что на последнем ходу, если он может забрать остатки, он забирает
...
bot_question = input('Вы хотите сыграть с ботом?, если да то введите "да", если нет то введите любой сивол ')
bot_game = False if bot_question.lower == 'да' else True
fate = randint(1,2)
if bot_game:
...
{max_candy_round if max_candy_round < total_candy else total_candy} конфет - '))
                ...
                if 0 < first_player_candy <= (max_candy_round if max_candy_round < total_candy else total_candy):
                    print(f'{first_player_name} берёт берёт - {first_player_candy} конфет')
                    break
...
{max_candy_round if max_candy_round < total_candy else total_candy} конфет - '))
                ...
                if 0 < second_player_candy <= (max_candy_round if max_candy_round < total_candy else total_candy):
                    print(f'{second_player_name} берёт берёт - {second_player_candy} конфет')
                    break
